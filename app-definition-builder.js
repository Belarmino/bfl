...
angular.module('flowableModeler')
  .controller('AppDefinitionBuilderController', ['$rootScope', '$scope', '$translate', '$http', '$location', '$routeParams', '$modal', '$popover', '$timeout','UserService','GroupService', 
                              function ($rootScope, $scope, $translate, $http, $location, $routeParams, $modal, $popover, $timeout,UserService,GroupService) {
	...
	/****/
	$scope.openUserGroup = function() {
		_internalCreateModal({
            template: 'views/popup/assignment-popup.html?version=' + Date.now(),
            scope: $scope
        }, $modal, $scope);
	};
	$scope.filter = {users:[],group:[],usersAss:[],groupAss:[]};
	$scope.findUser = function() {
		var data = UserService.getFilteredUsers($scope.filter.usersAccess).$$state;
		//console.log(data);
		$scope.filter.users = data;
	}
	$scope.findGroup = function() {
		var data = GroupService.getFilteredGroups($scope.filter.groupsAccess).$$state;
		//console.log(data);
		$scope.filter.groups = data;
	}
	$scope.selectUser = function(user) {
		console.log(user);
        $scope.filter.usersAccess=user.fullName;
		var _user = {"id":user.id,"fullName":user.fullName};
		if( $rootScope.currentAppDefinition.definition.usersAccess.indexOf(_user)==-1){
			$rootScope.currentAppDefinition.definition.usersAccess.push(_user);
			
		}
    };
	$scope.removeUser = function(user) {
        var index = $rootScope.currentAppDefinition.definition.usersAccess.indexOf(user);
		if(index!=-1){
			$rootScope.currentAppDefinition.definition.usersAccess.splice(index, 1);
		}
    };
	$scope.selectGroup = function(group) {
		console.log(group);
		$scope.filter.groupsAccess=group.name;
		var _group = {"id":group.id,"name":group.name};
		if($rootScope.currentAppDefinition.definition.groupsAccess.indexOf(_group)==-1){
			$rootScope.currentAppDefinition.definition.groupsAccess.push(_group);
		}
    };
	$scope.removeGroup = function(group) {
        var index = $rootScope.currentAppDefinition.definition.groupsAccess.indexOf(group);
		if(index!=-1){
			$rootScope.currentAppDefinition.definition.groupsAccess.splice(index, 1);
		}
    };
    ...
    $scope.loadApp = function() {
    	$http({method: 'GET', url: FLOWABLE.CONFIG.contextRoot + '/app/rest/app-definitions/' + $routeParams.modelId}).
        	success(function(data, status, headers, config) {
				...
				$rootScope.currentAppDefinition.definition.usersAccess=[];
				$rootScope.currentAppDefinition.definition.groupsAccess=[];
        ...
        for (var i = 0; i < $scope.popup.models.data.length; i++) {
            if ($scope.popup.selectedModels.indexOf($scope.popup.models.data[i].id) >= 0) {
                ...
                var summaryModel = {
					...
					groupsAccess: selectedModel.groupsAccess,
                    usersAccess: selectedModel.usersAccess
                };
